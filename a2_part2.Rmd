...
cohens_d <- function(x1, x2, sd1, sd2, n1, n2){
  ...
  pooled_sd <- pooled_sd_nominator / pooled_sd_denominator

  return(mean_diff / pooled_sd)
}

mean_diff_se <- function(sd1, sd2, n1, n2){
  sd1 / sqrt(n1) + sd2 / sqrt(n2)
}
...
ggplot(data %>% filter(diagnosis == 'sz')) +
...
fit_data_trimmed <- fit_data_all %>% 
  filter(abs(effect) < 1.5*IQR(effect))
```
