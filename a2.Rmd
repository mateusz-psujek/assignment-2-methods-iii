...
```{r}
# saving the object from part 1. In case i have to use them I won't need to rerun the whole thing
...
data <- data_raw %>% 
  ...
  mutate_all(~ str_to_lower(.x) %>%  
               na_if('nr')) %>% 
  mutate(across(!1:3, ~ str_replace_all(.x, ',', '.') %>% str_remove_all("[^0-9.]") %>% 
                        as.numeric)) %>% #there were weird cells like '2,63197\r\n \r\n	' that I needed to fix before converting to numeric
  add_count(studyid) %>% 
  ...
  group_by(studyid) %>% 
  mutate(sample_size = sum(n_diagnosis), .after = n_diagnosis) %>% 
  ungroup
...
```{r}
#sample size
data %>% ggplot(aes(x = sample_size)) +
  geom_histogram(fill = 'navy', color = 'black', alpha = 0.1) +
  geom_vline(aes(xintercept = mean(sample_size, na.rm = T), color = 'mean sample size'),
               linetype = 'dashed', size = 0.6) +
  theme_minimal() +
  labs(x = "Sample size \n (both healthy and schizophrenic participants)",
       y = "Count") +
  scale_color_manual(name = element_blank(), values = c(`mean sample size` = 'navy'))  


data %>% ggplot(aes(x = sample_size)) +
  geom_histogram(fill = 'navy', color = 'black', alpha = 0.1) +
  geom_vline(aes(xintercept = mean(sample_size), color = 'mean sample size'),
