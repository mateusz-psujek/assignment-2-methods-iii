...
```
```{r}
# to i want to save all levels? is so in one tibble?
# End goal: 
        # study     # mean    # sigma

simulate_data <- function(n = 100, gt_mean = 0.4, gt_sd = 0.4, error = 0.8, seed = 1)
  #expaning the names: n - number of studies, gt stands for 'ground-truth', mes stands for measurement or measured
  {
  set.seed(seed)
  ...
  data <- tibble(study = seq(1, n, by = 1)) %>% 
    ...
    mutate(t_effect = rnorm(1, gt_mean, gt_sd) %>% round(2), 
           ...
           pub = if_else(signif == 'yes' & effect > 0, rbinom(1, 1, 0.9), rbinom(1, 1, 0.2))) %>% 
    ungroup %>% 
...
effect_hist(check)
effect_hist(check %>% filter(pub == 1))
...
```{r}
rbind(gather_draws(m_all, c(b_Intercept, sd_study__Intercept)) %>% mean_qi, 
