...
pop_effects_plot <- function(models, model_names, null = 0)
  ...
  plot_data %>% 
    ggplot(aes(x = .value, y = model, xmin = .lower, xmax = .upper)) +
      geom_pointrange() +
      geom_vline(xintercept = null, linetype = 'dashed', color = 'darkred', size = 0.5) + 
      theme_minimal() +
      labs(x = NULL,
           y = NULL,
...
```{r}
pop_effects_plot(models = list(m_all, m_pub), model_names = c('Without publication bias', 'With publication bias')) +
...
```
```{r fig.height = 8}
rand_effects_plot(models = list(m_all, m_pub), model_names = c('Without publication bias', 'With publication bias'))
...
plot_data <- map2_df(
    .x = list(m_all, m_pub), .y = c('Without publication bias', 'With publication bias'),
    ~ bind_rows(plot_data,
                gather_draws(.x, r_study[study, parameter]) %>% mean_qi %>% mutate(model = .y)
                )
    )
...
plot_data <- plot_data %>% 
      mutate(
        t_value = c(data$t_effect, data_pub$t_effect),
        accuracy = if_else(t_value < .upper & t_value > .lower, 'Accurate', 'Inaccurate'))
  
plot_data %>% 
   ggplot(aes(x = .value, y = study, xmax = .upper, xmin = .lower, colour = accuracy)) +
     geom_pointrange() +
     scale_color_manual(values = c('darkseagreen4', 'darkred')) +
     labs(title = 'True effect sizes vs. model estimates',
          subtitle = '95% confidence intervals of the estimated Intercepts', 
          x = NULL,
          y = 'Study')
